...
//         setCategories([{ categoryId: "0", categoryName: "All Categories", description: "All products", status: "Active" }, ...activeCategories]);
//       } catch (err) {
...
//             axios.get("http://localhost:8080/auth/getallbooks"),
//             axios.get("http://localhost:8080/auth/getallaccessories"),
//           ]);
//           books = booksResponse.data.filter(book => book.status === "Active");
//           accessories = accessoriesResponse.data.filter(acc => acc.status === "Active");
//         } else {
//           const category = categories.find(cat => cat.categoryId === selectedCategory);
//           if (category?.type === "book") {
//             const response = await axios.get(`http://localhost:8080/auth/getbooksbycategoryid/${selectedCategory}`);
//             books = response.data.filter(book => book.status === "Active");
//           } else if (category?.type === "accessory") {
//             const response = await axios.get(`http://localhost:8080/auth/getaccessoriesbycategoryid/${selectedCategory}`);
//             accessories = response.data.filter(acc => acc.status === "Active");
//           }
//         }
...
//         setProducts([...normalizedBooks, ...normalizedAccessories]);
//       } catch (err) {
...
//     return matchesSearch && product.status === "Available";
//   });
...
//       }, 1000); // Increased to 1000ms for backend sync
//     } catch (err) {
...
//         <div className="w-full h-full bg-gradient-to-b from-white via-white via-70% to-blue-50 [clip-path:polygon(0_0,100%_0,100%_85%,0_100%)]"></div>
//       </div>
...
//               Discover our curated selection of books and stationery for all ages and interests.
//             </p>
//           </div>
...
//               className="w-full sm:w-1/3 px-4 py-2 bg-white border border-yellow-600/50 text-black rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-600 font-sans transition-all duration-300"
//             >
...
//               placeholder="Search books or accessories..."
//               className="w-full sm:w-1/3 px-4 py-2 bg-white border border-yellow-600/50 text-black rounded-md focus:outline-none focus:ring-2 focus:ring-yellow-600 font-sans transition-all duration-300"
//             />
//           </div>
...
const ProductList = () => {
  ...
  const addToGuestCart = (product) => {
    ...
    const existingIndex = guestCart.findIndex(
      (item) => item.productId === product.id && item.productType === product.type
    );
    let newCart;
    if (existingIndex !== -1) {
      newCart = [...guestCart];
