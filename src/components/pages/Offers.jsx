...
//   const currentDate = new Date("2025-07-12T20:23:00+05:30");
...
//       try {
//         // Fetch categories
//         const categoriesResponse = await axios.get(`${API_URL}/auth/getallcategories`);
//         const activeCategories = categoriesResponse.data.filter(cat => cat.status === "Active");
//         setCategories([{ categoryId: "0", categoryName: "All Categories", description: "All products", status: "Active" }, ...activeCategories]);

//         // Fetch all products (books and accessories) with discounts
//         const [booksResponse, accessoriesResponse] = await Promise.all([
//           axios.get(`${API_URL}/auth/getallbooks`),
//           axios.get(`${API_URL}/auth/getallaccessories`),
//         ]);

//         const books = booksResponse.data
//           .filter(book => book.status === "Active" && book.discount > 0)
//           .map(book => ({
//             id: book.bookId,
...
//           .filter(acc => acc.status === "Active" && acc.discount > 0)
//           .map(acc => ({
//             id: acc.id,
...
//         setProducts([...books, ...accessories]);
//       } catch (err) {
//         console.error("Fetch error:", err.response?.data || err.message);
//         setError(err.response?.data?.message || "Failed to load offers or categories.");
//       } finally {
//         setLoading(false);
//       }
//     };
