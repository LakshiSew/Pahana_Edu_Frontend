...
const Product = () => {
  ...
  const getDiscountedPrice = (priceString, discount) => {
    const price = parseFloat(priceString.replace("LKR ", ""));
    return discount > 0 ? `LKR ${(price * (1 - discount / 100)).toFixed(2)}` : priceString;
  };
  ...
  const addToGuestCart = (product) => {
    ...
    if (existingIndex !== -1) {
      ...
      const originalPrice = parseFloat(product.price.replace("LKR ", ""));
      const discountedPrice = product.currentPrice
        ? parseFloat(product.currentPrice.replace("LKR ", ""))
        : originalPrice;
      newCart = [
        ...guestCart,
        {
