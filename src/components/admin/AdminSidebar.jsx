...
//     { icon: FileText, label: "Accessories", path: "/admin/accessories" },
...
//           {!collapsed && (
//             <div>
//               <p className="text-sm font-medium text-white font-sans">Admin Name</p>
//               <p className="text-xs text-yellow-400 font-sans">Administrator</p>
//             </div>
//           )}
//         </div>
//         <motion.button
  ...
  useEffect(() => {
    const fetchAdminDetails = async () => {
      try {
        ...
        const token = localStorage.getItem("token");
        if (!userId || !token) {
          toast.error("Please log in to view admin details", {
            position: "top-right",
            autoClose: 3000,
          });
          setLoading(false);
        ...
        if (admin.position !== "Manager") {
          toast.error("Access denied: Not a manager", {
            position: "top-right",
            autoClose: 3000,
          });
          setLoading(false);
          ...
          {!collapsed && (
            <div>
              <p className="text-sm font-medium text-white font-sans">
                {loading ? "Loading..." : adminDetails.userName}
              </p>
              <p className="text-xs text-yellow-400 font-sans">Manager</p>
            </div>
          )}
        </div>
        <motion.button
